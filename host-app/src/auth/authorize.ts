...
export type Decision =
  | { outcome: 'allow'; because: 'public' | 'membership' | 'repo-access' }
 ...
 *   3. A share link scoping this record -> PHASE 3. Deliberately absent rather than stubbed: an
 *      always-false branch reads as implemented, and this file is the wrong one to guess in.
...
export async function authorize(
  ...
  const project = await deps.projects.byRepo(repo)
  ...
  // FROM HERE THE PROJECT IS PRIVATE. Everything below decides whether this particular person gets
  // in, and every path out of it that is not `allow` must be indistinguishable to them.
  ...
  const verdict = await canReadRepo(viewer.userId, project.repo, deps.access, deps.now ?? new Date())
