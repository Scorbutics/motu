...
/**
 * `writeIfChanged` comes from the CLI's contribute context, and using it is not housekeeping: this file
 * lands in `.motu/cache`, which Vite WATCHES, and it is regenerated on every lagoon build. Writing the
 * same bytes still bumps mtime, which still full-reloads every open lagoon tab — so a check running in
 * another terminal used to reload the page a person was looking at. See the CLI's write-if-changed.mjs.
 */
function generateTailwindConfig(paths, writeIfChanged) {
...
export async function contribute({ paths, lagoonJson, resolveBuildDep, writeIfChanged }) {
  ...
  return {
    ...
    css: {
      postcss: {
        plugins: [
          ...(usesTailwind ? [tailwindcss({ config: generateTailwindConfig(paths, writeIfChanged) })] : []),
          autoprefixer(),
        ],
      },
    },
